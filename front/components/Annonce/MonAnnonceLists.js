...
const MonAnnonceLists = ({ dispatch,
    ...
    const handleDelete = async (id)=>{
        try {
            //await deleteAnnonceList(id);
            alert("Votre annonce ete delete");
        } catch (err) {
            console.log(err);
        }
    }
                            ...
                            <span className="text-sm block my-4 p-3 text-gray-800 rounded border border-solid border-gray-200">
							    <div className="top justify-between">
                                  <div className="font-bold text-2xl uppercase text-orange-700 text-center py-2 m-2">
                                     {car?.brand}  {car?.generation} | <i className="far fa-clock"></i> <Moment format="DD/MM/YYYY">{car?.expire_at}</Moment>
                                  </div>
                                  <div className="price font-bold text-orange-500  text-2xl text-center bg-gray-400 px-4 py-2">
                                    {car?.estimate_price} €
                                  </div>
                                </div>
                              ...
                              <div className="flex flex-wrap">
                                    ...
                                    <div className="w-1/3">
                                      <span className="text-sm block my-4 p-3 text-gray-800 rounded border border-solid border-gray-200"><i
                                          className="fas fa-calendar-check"></i> <Moment format="DD/MM/YYYY">{car.created_at}</Moment></span>
                                    </div>
                                  </div>
                            </div>
